...
const generateMarkdown = function(filename, schema, schemaPath, outDir, dependencyMap, docs) {
  ...
  function getExamples(_schema) {
    if (_schema.examples) {
      return _schema.examples[0];
    } else if (_schema.properties) {
      return _.mapValues(_schema.properties, _subSchema => {
        return getExamples(_subSchema);
      });
    } else if (_schema.items) {
      return [ getExamples(_schema.items) ];
    } else {
      return '#### EMPTY ####';
    }
  }

  const schemaExample = getExamples(schema);
  ...
  if (_.keys(schema.properties).length > 0) {
    ...
    for (let i=0; i<_.keys(schema.properties).length;i++) {
      const name = _.keys(schema.properties)[i];
      multi.push( [ 'property.ejs', {
        name: name,
